...
export function resolveMidWashout(drug, perDrugInput) {
  ...
  if (drug.specialInput === "doac-dose") {
    const dose = perDrugInput?.dose || "high";
    return drug.washoutByDose[dose];
  }
...
export function resolveCatheterRestart(drug, perDrugInput) {
  if (drug.specialInput === "doac-dose") {
    const dose = perDrugInput?.dose || "high";
    return drug.catheterRestartByDose[dose];
  }
  return drug.catheterRestart;
}

export function aggregateCatheterRestart(selectedDrugs, inputsById = {}) {
  if (selectedDrugs.length === 0) {
    return { status: "none" };
  }
  const resolved = selectedDrugs.map((drug) => {
    const perDrugInput = inputsById[drug.id] || {};
    const restart = resolveCatheterRestart(drug, perDrugInput);
    return { drug, restart, unit: getRestartUnit(restart) };
  });
